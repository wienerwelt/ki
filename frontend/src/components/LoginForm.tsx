...
import {
  ...
  CircularProgress, LinearProgress, Divider, useTheme
} from '@mui/material';
  ...
  return (
    <>
      <Box component="form" onSubmit={handleSubmit} noValidate sx={{ width: '100%', mt: 1 }}>
        ...
        {!isRegister && (
          <TextField
            margin="normal" required fullWidth id="identifier" label="Benutzername oder E-Mail"
            name="identifier" autoComplete="username" autoFocus
            value={username || email} onChange={(e) => setUsername(e.target.value)}
            disabled={loading}
          />
        )}

        <TextField
          margin="normal" required fullWidth name="password" label="Passwort"
        ...
        {isRegister && (
          <FormControlLabel
            control={<Checkbox checked={consent} onChange={(e) => setConsent(e.target.checked)} sx={{ '&.Mui-checked': { color: customPrimaryColor } }} />}
            label={
              <Typography variant="body2">
                Ich stimme den{' '}
                <Link component="button" type="button" onClick={() => handleOpenLegalDialog('terms')} sx={{ verticalAlign: 'baseline', color: customPrimaryColor }}>AGB</Link>,{' '}
                <Link component="button" type="button" onClick={() => handleOpenLegalDialog('privacy')} sx={{ verticalAlign: 'baseline', color: customPrimaryColor }}>Datenschutz</Link>{' & '}
                <Link component="button" type="button" onClick={() => handleOpenLegalDialog('disclaimer')} sx={{ verticalAlign: 'baseline', color: customPrimaryColor }}>Disclaimer</Link> zu.
              </Typography>
            }
        ...
        {isRegister && (
          <FormControlLabel
            control={<Checkbox checked={newsletterOptIn} onChange={(e) => setNewsletterOptIn(e.target.checked)} sx={{ '&.Mui-checked': { color: customPrimaryColor } }} />}
        ...
        <Button
          ...
          size="large"
          sx={{ 
            mt: 3, mb: 2, py: 1.5, fontWeight: 'bold', fontSize: '1rem',
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            {!isRegister && (
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Link 
                      component={RouterLink} to="/forgot-password" variant="body2" underline="hover"
                    ...
                    <Link 
                      ...
                      onClick={() => { setResendEmail(''); setResendOpen(true); }} 
        ...
        <Typography variant="body2" align="center" color="text.secondary">
            <Link 
              href="https://www.mobiliti.at" target="_blank" rel="noopener noreferrer" underline="hover"
              sx={{ color: customPrimaryColor }}
            >
                mobiliti.at
      ...
      <Dialog open={resendOpen} onClose={() => setResendOpen(false)} fullWidth maxWidth="xs">
        ...
        <DialogActions>
          ...
          <Button 
            ...
            sx={{ backgroundColor: customPrimaryColor, color: customTextColor, '&:hover': { backgroundColor: customPrimaryColor, filter: 'brightness(0.9)' } }}
          >
            Senden
          </Button>
        </DialogActions>
