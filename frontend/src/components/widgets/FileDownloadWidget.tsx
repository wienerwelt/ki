...
import axios from 'axios'; // WICHTIG: Fehlenden Import hinzugefügt
import {
    Box, Typography, CircularProgress, Alert, Grid, Paper, IconButton,
    Tooltip, TextField, InputAdornment, Chip, FormControl, InputLabel, Select, MenuItem,
    List, ListItem, ListItemIcon, ListItemText, SelectChangeEvent, ToggleButtonGroup, ToggleButton
} from '@mui/material';
...
interface FileDownloadWidgetProps extends BaseWidgetProps {
    title: string;
    ...
    <Grid item xs={12} sm={6} md={4} lg={3}>
        <Paper variant="outlined" sx={{ p: 2, height: '100%', display: 'flex', flexDirection: 'column' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1 }}>
                ...
                <Typography variant="subtitle1" sx={{ flexGrow: 1, wordBreak: 'break-word', fontWeight: 'bold' }}>
                    {file.filename}
                </Typography>
            </Box>
            <Typography variant="body2" color="text.secondary" sx={{ flexGrow: 1, minHeight: '40px' }}>
                {file.description || `Hochgeladen am ${new Date(file.created_at).toLocaleDateString('de-DE')}`}
            </Typography>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 2, pt: 1, borderTop: 1, borderColor: 'divider' }}>
                ...
                {file.download_count !== undefined && (
                    <Typography variant="caption" color="text.secondary">{file.download_count} Downloads</Typography>
                )}
...
const FileListItem: React.FC<{ file: PartnerFile, onDownload: (id: string, name: string) => void }> = ({ file, onDownload }) => (
    <ListItem secondaryAction={<Tooltip title="Herunterladen"><IconButton edge="end" onClick={() => onDownload(file.id, file.filename)}><DownloadIcon /></IconButton></Tooltip>}>
        ...
        <ListItemText
            ...
            secondary={`Größe: ${formatFileSize(file.file_size)}${file.download_count !== undefined ? ` • ${file.download_count} Downloads` : ''} • ${new Date(file.created_at).toLocaleDateString('de-DE')}`}
        />
    </ListItem>
);
...
const FileDownloadWidget: React.FC<FileDownloadWidgetProps> = ({ onDelete, widgetId, isRemovable, title, widgetTypeKey }) => {
    ...
    const sortedAndFilteredFiles = useMemo(() => {
        ...
        let filtered = files.filter(file =>
            ...
            file.tags?.some(tag => tag.toLowerCase().includes(lowercasedFilter))
        );
