...
import { Container, Box, CircularProgress, Alert, Typography, Menu, MenuItem, Button } from '@mui/material';
...
// === GEÄNDERT: Zentrale Mappings vom korrekten Ort importieren ===
...
const DashboardPage: React.FC = () => {
    ...
    const renderWidgetContent = (widget: WidgetConfig) => {
        ...
        if (!widgetTypeMeta) {
            return <Box p={2}><Typography>Unbekannter Widget-Typ: {widget.type}</Typography></Box>;
        }

        const componentKey = widgetTypeMeta.component_key || widget.type;
        const SpecificWidgetComponent = WIDGET_COMPONENTS[componentKey];
        const IconComponent = getIcon(widgetTypeMeta.icon_name);

        if (!SpecificWidgetComponent) {
            console.error(`Widget-Komponente für Key '${componentKey}' nicht in widgetMapping gefunden.`);
            return <Box p={2}><Typography>Fehlende Komponente für: {widget.type}</Typography></Box>;
        }
        ...
        const specificProps: { [key: string]: any } = {
            'GenericAI': { category: config.category, filterLabel: config.filterLabel, description: widgetTypeMeta.description },
            'GenericScrape': { category: config.category, filterLabel: config.filterLabel, description: widgetTypeMeta.description },
            'BusinessPartnerInfo': { businessPartner: businessPartner },
        };
