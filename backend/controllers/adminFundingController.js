...
exports.getAllFundingOpportunities = async (req, res) => {
    ...
    try {
        ...
        if (categoryId) {
            ...
            // Diese Bedingung prüft, ob die gegebene Kategorie-ID im Array der Kategorien der Förderung enthalten ist.
            whereClauses.push(`f.id IN (SELECT opportunity_id FROM funding_opportunities_categories WHERE category_id = $${queryParams.length})`);
        }
...
// --- NEUE FUNKTION, um eine Liste der Quellen für den Filter bereitzustellen ---
exports.getFundingSourceRules = async (req, res) => {
    try {
...
exports.updateFundingOpportunity = async (req, res) => {
    ...
    try {
        ...
        const { rows: [updatedOpportunity] } = await client.query(
            ...
            [title, deadline_end || null, status, region, id]
        );
...
exports.deleteFundingOpportunity = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await db.query('DELETE FROM funding_opportunities WHERE id = $1 RETURNING id', [id]);
...
exports.getAllFundingCategories = async (req, res) => {
    try {
...
exports.getFundingUsageStats = async (req, res) => {
    ...
    const { dateTrunc, interval } = allowedTimespans[timespan];
...
// Fügen Sie diese Funktion zu Ihrer adminFundingController.js hinzu
